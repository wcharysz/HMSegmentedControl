...
- (void)commonInit {
    ...
    self.selectionIndicatorBoxOpacity = 0.2f;
...
- (void)setSelectionIndicatorBoxOpacity:(float)selectionIndicatorBoxOpacity {
...
- (void)setSegmentWidthStyle:(HMSegmentedControlSegmentWidthStyle)segmentWidthStyle {
    ...
    _segmentWidthStyle = self.type == HMSegmentedControlTypeImages ? HMSegmentedControlSegmentWidthStyleFixed : segmentWidthStyle;
}
...
- (void)drawRect:(CGRect)rect {
    ...
    if (self.type == HMSegmentedControlTypeText) {
        [self.sectionTitles enumerateObjectsUsingBlock:^(NSAttributedString *titleString, NSUInteger idx, BOOL *stop) {
            
            CGFloat stringWidth = titleString.size.width;
            CGFloat stringHeight = titleString.size.height;
            ...
            CGFloat y = round((CGRectGetHeight(self.frame) - selectionStyleNotBox * self.selectionIndicatorHeight) / 2 - stringHeight / 2 + self.selectionIndicatorHeight * locationUp);
            CGRect newRect = rect;
            
            if (self.segmentWidthStyle == HMSegmentedControlSegmentWidthStyleFixed) {
                newRect = CGRectMake((self.segmentWidth * idx) + (self.segmentWidth - stringWidth) / 2, y, stringWidth, stringHeight);
                ...
                newRect = CGRectMake(xOffset, y, widthForIndex, stringHeight);
            ...
            titleLayer.frame = CGRectMake(ceil(newRect.origin.x), ceil(newRect.origin.y), ceil(newRect.size.width), ceil(newRect.size.height));
        ...
        [self.sectionImages enumerateObjectsUsingBlock:^(UIImage *icon, NSUInteger idx, BOOL *stop) {
            CGFloat imageWidth = icon.size.width;
            CGFloat imageHeight = icon.size.height;
            CGFloat y = round(CGRectGetHeight(self.frame) - self.selectionIndicatorHeight) / 2 - imageHeight / 2 + ((self.selectionIndicatorLocation == HMSegmentedControlSelectionIndicatorLocationUp) ? self.selectionIndicatorHeight : 0);
            CGFloat x = self.segmentWidth * idx + (self.segmentWidth - imageWidth)/2.0f;
            CGRect newRect = CGRectMake(x, y, imageWidth, imageHeight);
            
            CALayer *imageLayer = [CALayer layer];
            imageLayer.frame = newRect;
            imageLayer.contents = (id)icon.CGImage;
            
            if ([@(self.selectedSegmentIndex) isEqualToNumber:@(idx)] && self.sectionSelectedImages) {
        ...
        [self.sectionImages enumerateObjectsUsingBlock:^(UIImage *icon, NSUInteger idx, BOOL *stop) {
            ...
            CGFloat stringHeight = self.sectionTitles[idx].size.height;
            CGFloat yOffset = round(((CGRectGetHeight(self.frame) - self.selectionIndicatorHeight) / 2) - (stringHeight / 2));
            ...
            CGFloat imageYOffset = round((CGRectGetHeight(self.frame) - self.selectionIndicatorHeight) / 2.0f);
            ...
            // Fix rect position/size to avoid blurry labels
            CATextLayer *titleLayer = [CATextLayer layer];
            titleLayer.frame = CGRectMake(ceil(textRect.origin.x), ceil(textRect.origin.y), ceil(textRect.size.width), ceil(textRect.size.height));
            titleLayer.alignmentMode = kCAAlignmentCenter;
    ...
    if (self.selectedSegmentIndex < 0) {
        if (self.selectionStyle == HMSegmentedControlSelectionStyleArrow && !self.selectionIndicatorArrowLayer.superlayer) {
            [self setArrowFrame];
            [self.scrollView.layer addSublayer:self.selectionIndicatorArrowLayer];
        } else if (!self.selectionIndicatorStripLayer.superlayer) {
...
- (CGRect)frameForSelectionIndicator {
    ...
    CGFloat sectionWidth = self.selectedSegmentIndex < 0 ? 0 : self.sectionTitles[(NSUInteger)self.selectedSegmentIndex].size.width;
    CGFloat imageWidth = self.selectedSegmentIndex < 0 ? 0 : self.sectionImages[(NSUInteger)self.selectedSegmentIndex].size.width;
    
    if (self.type == HMSegmentedControlTypeImages) {
        sectionWidth = imageWidth;
    } else if (self.type == HMSegmentedControlTypeTextImages) {
        sectionWidth = MAX(sectionWidth, imageWidth);
    }
    ...
    if (self.selectionStyle == HMSegmentedControlSelectionStyleArrow) {
        ...
        if (self.selectionStyle == HMSegmentedControlSelectionStyleTextWidthStripe &&
            ...
            if (self.segmentWidthStyle == HMSegmentedControlSegmentWidthStyleDynamic) {
                ...
                CGFloat width = self.selectedSegmentIndex < 0 ? 0 : self.segmentWidthsArray[(NSUInteger)self.selectedSegmentIndex].floatValue;
                
                return CGRectMake(selectedSegmentOffset + self.selectionIndicatorEdgeInsets.left, indicatorYOffset, width - self.selectionIndicatorEdgeInsets.right, self.selectionIndicatorHeight + self.selectionIndicatorEdgeInsets.bottom);
            }
...
- (CGRect)frameForFillerSelectionIndicator {
    if (self.segmentWidthStyle == HMSegmentedControlSegmentWidthStyleDynamic) {
        ...
        return CGRectMake(selectedSegmentOffset, 0, width, CGRectGetHeight(self.frame));
    }
...
- (void)updateSegmentsRects {
    ...
    if (self.type == HMSegmentedControlTypeText && self.segmentWidthStyle == HMSegmentedControlSegmentWidthStyleFixed) {
        for (NSAttributedString *titleString in self.sectionTitles) {
            CGFloat stringWidth = titleString.size.width + self.segmentEdgeInset.left + self.segmentEdgeInset.right;
            self.segmentWidth = MAX(stringWidth, self.segmentWidth);
        }
    } else if (self.type == HMSegmentedControlTypeText && self.segmentWidthStyle == HMSegmentedControlSegmentWidthStyleDynamic) {
        NSMutableArray *mutableSegmentWidths = [[NSMutableArray alloc] initWithCapacity:self.sectionTitles.count];
        for (NSAttributedString *titleString in self.sectionTitles) {
            CGFloat stringWidth = titleString.size.width + self.segmentEdgeInset.left + self.segmentEdgeInset.right;
        ...
        for (NSAttributedString *titleString in self.sectionTitles) {
            CGFloat stringWidth = titleString.size.width + self.segmentEdgeInset.right;
            ...
            [mutableSegmentWidths addObject:@(combinedWidth)];
            i++;
        }
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    if (CGRectContainsPoint(enlargeRect, touchLocation)) {
        ...
        if (self.segmentWidthStyle == HMSegmentedControlSegmentWidthStyleFixed) {
            segment = @((touchLocation.x + self.scrollView.contentOffset.x) / self.segmentWidth).integerValue;
        } else {
        ...
        NSUInteger sectionsCount = self.type == HMSegmentedControlTypeImages ? self.sectionImages.count : self.sectionTitles.count;
        
        if (self.isTouchEnabled && segment != self.selectedSegmentIndex && segment < (NSInteger)sectionsCount) {
...
- (void)scrollToSelectedSegmentIndex:(BOOL)animated {
    ...
    if (self.segmentWidthStyle == HMSegmentedControlSegmentWidthStyleFixed) {
        ...
        CGFloat width = self.selectedSegmentIndex < 0 ? 0 : self.segmentWidthsArray[(NSUInteger)self.selectedSegmentIndex].floatValue;
        rectForSelectedIndex = CGRectMake(offsetter, 0, width, self.frame.size.height);
        selectedSegmentOffset = (CGRectGetWidth(self.frame) / 2) - (width / 2);
    }
...
- (void)setSelectedSegmentIndex:(NSInteger)index animated:(BOOL)animated notify:(BOOL)notify {
    ...
    if (index < 0) {
        ...
        if (animated) {
            ...
            if (notify) {
                [self notifyForSegmentChangeToIndex:(NSUInteger)index];
            }
            ...
            [self setArrowFrame];
            CGRect selectionFrame = [self frameForSelectionIndicator];
            self.selectionIndicatorBoxLayer.frame = selectionFrame;
            self.selectionIndicatorStripLayer.frame = selectionFrame;
            self.selectionIndicatorBoxLayer.frame = [self frameForFillerSelectionIndicator];
            [CATransaction commit];
        } else {
            // Disable CALayer animations
            NSDictionary *newActions = @{@"position": [NSNull null], @"bounds": [NSNull null]};
...
- (void)notifyForSegmentChangeToIndex:(NSUInteger)index {
    if (self.superview)
